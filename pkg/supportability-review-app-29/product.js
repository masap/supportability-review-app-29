...
export function init($plugin, store) {
  ...
  function getCheckResult(row) {
    if (row.status === undefined || row.status.reportJSON === undefined) return '---'

    try {
      const data = JSON.parse(row.status.reportJSON)
      let cnt_pass = 0
      let cnt_fail = 0
      let cnt_warn = 0
      let cnt_skip = 0
      data.clusters.forEach((cluster) => {
        cluster.groups.forEach((group) => {
          cnt_pass += group.checks_pass
          cnt_fail += group.checks_fail
          cnt_warn += group.checks_warn
          cnt_skip += group.checks_skip
        })
      })
      return cnt_pass + '/' + (cnt_pass + cnt_fail + cnt_skip)
    } catch (error) {
      return '---'
    }
  }
  ...
  headers(SUPPORTABILITY_REVIEW_CRD_IDS.REVIEW_BUNDLE, [
    ...
    },
    {
      ...
      getValue: (row) => getCheckResult(row),
      sort: ['status.clusterCount']
    },
    {
      name: 'Size',
